	...
	<div class='column-group'>
		...
		<form
			...
			<c:if test="${action == 'edit'}">
				<input type="hidden" name="babysitterId"
					value="${babysitter.babysitterId}">
			</c:if>

			<label for="Name">Name</label> <input type="text" id="name"
				name="name" value="${name != null ? name : babysitter.name}"
				required>
			...
			<label for="age">Age</label> <input type="number" id="age" name="age"
				value="${age!=null? age:babysitter.age}" required>
			...
			<label for="experience">Experience</label> <input type="number"
				...
				value="${experience!=null? experience:babysitter.experience }"
				required>
			...
			<label for="email">Email</label> <input type="email" id="email"
				name="email" value="${email!=null? email:babysitter.email}" required>
			...
			<label for="contact">Contact number</label> <input type="tel"
				...
				value="${contact!=null? contact:babysitter.contactNumber}" required>
			...
			<label for="status">Status</label> <select id="status" name="status">
				<option value="available"
					${babysitter.status == 'available' ? 'selected' : ''}>Available</option>
				<option value="unavailable"
					${babysitter.status == 'unavailable' ? 'selected' : ''}>Unavailable</option>
			</select>
			<div class="form-submit">
				<button type="submit" class="form-button">${action == 'edit' ? 'Update Babysitter' : 'Add Babysitter'}
				</button>
			</div>
